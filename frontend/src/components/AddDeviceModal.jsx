...
const AVAILABLE_SENSORS = [
  { code: 'sensor_1', name: 'Sensor 1 - Temperature', unit: '°C' },
  { code: 'sensor_2', name: 'Sensor 2 - Humidity', unit: '%' },
  { code: 'sensor_3', name: 'Sensor 3 - Soil Moisture', unit: '%' },
  { code: 'sensor_4', name: 'Sensor 4 - Light Intensity', unit: 'lux' },
  { code: 'sensor_5', name: 'Sensor 5 - Pressure', unit: 'hPa' },
]
...
export default function AddDeviceModal({ isOpen, onClose, onAdd, isLoading }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.sensorCode.trim()) return setError('Sensor code is required')
            ...
            {!useCustomCode ? (
              ...
              <>
                <input
                  ...
                  value={formData.sensorCode}
                  onChange={handleChange}
                  placeholder="e.g., bedroom_temp, garage_humidity"
                  className="w-full bg-dark-900 border border-gray-700 rounded-lg px-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-neon-cyan/60 transition-colors"
                />
                <button
                  type="button"
